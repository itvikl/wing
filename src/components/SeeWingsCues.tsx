...
function cueOpacity(progress: number, index: number, count: number) {
...
function CueBlock({
  ...
  const opacity = useTransform(progress, (p) => cueOpacity(p, index, count));
  const y = useTransform(progress, (p) => `${(1 - cueOpacity(p, index, count)) * -48}px`);
